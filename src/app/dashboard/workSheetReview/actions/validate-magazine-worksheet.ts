...
export const ValidateMagazineWorkSheet = async (payload: MagazineTextValues, magazineId: string | string[]) => {
    ...
    try {
        console.log(payload)
        if (USE_INGESTION) {
            const responseReview = await ingestionAutoReview('magazine', magazineId);
            ...
            const responseUpload = await ingestionAutoUpload('magazine', magazineId);
            ...
            const responseValidate = await apiFetch(`/cards/save-texts/${magazineId}`, {
                method: 'POST',
                body: { ...payload },
            });

            const responseNeo4j = await apiFetch(`/cards/upload/magazine/${magazineId}`, {
                method: 'PUT',
            });
