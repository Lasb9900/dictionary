...
export const reopenWorksheet = async (id: string, observation: string) => {
    try {
        ...
        if (!response.ok) {
            return {
                ok: false,
                message: response.message || 'No se pudo reabrir la ficha',
            };
        }
        ...
        return {
            ok: false,
            message: 'No se pudo reabrir la ficha',
        };
    }
};
